...
const GameMap = ({ player, npcs, enemies, onNPCInteract, onEnemyClick, onFountainUse, onCoalMineInteract, currentLocation, onPortalUse }: GameMapProps) => {
  ...
  // Handle cheat code for light (123 keys) and portal activation (E/У keys)
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      setPressedKeys(prev => new Set(prev).add(e.key.toLowerCase()));
      
      // Check for portal activation with E or У key
      if (e.key.toLowerCase() === 'e' || e.key.toLowerCase() === 'у') {
        if (isNearPortal()) {
          onPortalUse();
        } else if (isNearCoalMine()) {
          onCoalMineInteract();
        } else if (isNearOreMine()) {
          onCoalMineInteract(); // Используем ту же функцию для рудной шахты
        } else if (isNearFountain()) {
          onFountainUse();
        } else {
  ...
  }, [isNearPortal, isNearCoalMine, isNearOreMine, isNearFountain, onPortalUse, onCoalMineInteract, onFountainUse, onNPCInteract, npcs, player.position]);
  ...
  // Memoize torch positions to prevent regeneration on every render
  const torchPositions = useMemo(() => {
    if (currentLocation !== 'abandoned-mines') return [];
    
    const TORCH_COUNT = 24;
    const newTorches: { x: number; y: number }[] = [];
    const walls = minesObstacles.filter(r => r.w > 0 && r.h > 0);
    
    for (let i = 0; i < TORCH_COUNT && walls.length > 0; i++) {
      const wall = walls[Math.floor(Math.random() * walls.length)];
      // Determine orientation by comparing dimensions
      const isVertical = wall.h >= wall.w;
      if (isVertical) {
        const y = wall.y + Math.random() * wall.h;
        const x = wall.x + wall.w / 2;
        newTorches.push({ x: Math.round(x), y: Math.round(y) });
      } else {
        const x = wall.x + Math.random() * wall.w;
  ...
  // Memoize SVG mask to prevent recreation on every render
  const darknessOverlay = useMemo(() => {
    if (currentLocation !== 'abandoned-mines' || isLightCheatEnabled) return null;
    ...
    return (
      ...
      >
        <defs>
          <mask id="light-mask">
            {/* White = show overlay (darkness), Black = hide overlay (light) */}
            <rect x="0" y="0" width={mapWidth} height={mapHeight} fill="white" />
            {/* Player light - 300px radius with gradient */}
            <circle cx={player.position.x} cy={player.position.y} r={300} fill="url(#player-gradient)" />
            {/* Torch lights - 150px radius each with gradient */}
            {torchPositions.map((t, idx) => (
              <g key={idx}>
                <circle cx={t.x} cy={t.y} r={150} fill="url(#torch-gradient)" />
              </g>
            ))}
            {/* Gradient definitions */}
            <radialGradient id="player-gradient" cx="0.5" cy="0.5" r="0.5">
              <stop offset="0%" stopColor="black" stopOpacity="1" />
              <stop offset="66%" stopColor="black" stopOpacity="0" />
              <stop offset="100%" stopColor="black" stopOpacity="0" />
            </radialGradient>
            <radialGradient id="torch-gradient" cx="0.5" cy="0.5" r="0.5">
              <stop offset="0%" stopColor="black" stopOpacity="1" />
              <stop offset="66%" stopColor="black" stopOpacity="0" />
              <stop offset="100%" stopColor="black" stopOpacity="0" />
            </radialGradient>
        ...
        <rect x="0" y="0" width={mapWidth} height={mapHeight} fill="rgba(0,0,0,1)" mask="url(#light-mask)" />
      </svg>
    );
  }, [currentLocation, isLightCheatEnabled, player.position.x, player.position.y, torchPositions, cameraOffsetX, cameraOffsetY, zoomLevel, mapWidth, mapHeight]);
  ...
  // Walking animation frames with state to trigger re-renders
  const [walkFrameIndex, setWalkFrameIndex] = useState(0);
  
  useEffect(() => {
    const interval = setInterval(() => {
      setWalkFrameIndex(prev => (prev + 1) % 3);
    }, 200);
  ...
  const walkFrames = useMemo(() => ({
    down: ['walk_down1.png', 'walk_down2.png', 'walk_down3.png'][walkFrameIndex],
    up: ['walk_up1.png', 'walk_up2.png', 'walk_up3.png'][walkFrameIndex],
    side: ['walk_side1.png', 'walk_side2.png', 'walk_side3.png'][walkFrameIndex]
  }), [walkFrameIndex]);
  ...
  return (
    <div className={`flex-1 overflow-hidden relative cursor-crosshair ${
      ...
      {/* Darkness and lights for abandoned mines using SVG mask */}
      {darknessOverlay}
      ...
      >
        ...
        {/* Torch visuals */}
        {currentLocation === 'abandoned-mines' && torchPositions.map((t, idx) => (
          <div
            key={`torch-${idx}`}
            className="absolute"
            style={{ left: t.x - 4, top: t.y - 12 }}
            title="Факел"
          >
            <div
              ...
              style={{
                width: 8,
                height: 16,
                background: 'linear-gradient(180deg, #ffec85 0%, #ff9f1a 60%, #7a3e00 100%)',
                boxShadow: '0 0 10px 3px rgba(255, 165, 0, 0.9), 0 0 30px 10px rgba(255, 140, 0, 0.4)'
              }}
            />
          </div>
        ))}
      ...
      <img
        src={
          player.isMoving 
            ? (player.direction === 'down' ? `/${walkFrames.down}` 
               : player.direction === 'up' ? `/${walkFrames.up}`
               : player.direction === 'left' ? `/${walkFrames.side}`
               : player.direction === 'right' ? `/${walkFrames.side}`
               : "/walk_up2.png")
            : "/walk_up2.png"
        }
        alt="Player"
